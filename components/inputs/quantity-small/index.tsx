...
const QuantityInput: React.FC<QuantityInputProps> = ({
  ...
  return (
    <View className="flex-row items-center h-[28px]">
      {/* Nút giảm */}
      <TouchableOpacity
        onPress={handleDecrease}
        className={`w-8 h-full flex justify-center items-center rounded-l-md border-[1px] border-gray-200 ${
          quantity <= min ? "" : "bg-gray-100"
        }`}
        disabled={quantity <= min}
      >
        <Minus size={16} color={quantity > min ? "#000" : "#ccc"} />
      </TouchableOpacity>

      {/* Input */}
      <Input
        className="w-12 !h-full text-center text-sm border border-gray-200 rounded-none"
      ...
      <TouchableOpacity
        ...
        className={`w-8 h-full flex justify-center items-center rounded-r-md border-[1px] border-gray-200 ${
          quantity >= max ? "" : "bg-gray-100"
        }`}
        disabled={quantity >= max}
      >
        <Plus size={16} color={quantity < max ? "#000" : "#ccc"} />
      </TouchableOpacity>
    </View>
  );
};
